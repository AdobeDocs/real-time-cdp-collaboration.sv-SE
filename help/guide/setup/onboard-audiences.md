...
Innan du kan aktivera målgrupper med medarbetare och köra överlappningsberäkningar måste målgrupperna importeras till Real-Time CDP Collaboration. Om du vill importera målgrupper följer du arbetsflödesstegen i avsnittet nedan.

På fliken **[!UICONTROL My audiences]** på arbetsytan **[!UICONTROL Stetup]** väljer du ikonen Lägg till (![ikonen Lägg till).](/help/assets/icons/plus.png)) eller **[!UICONTROL Add]option** och välj sedan **Audience**.
...
>[!IMPORTANT]
>
>När du har upprättat till din första dataanslutning och importerat din första målgrupp kan du sedan importera flera målgrupper från den befintliga dataanslutningen. När du lägger till ytterligare målgrupper börjar du med steget [Välj målgrupp](#select-audience) eftersom all nödvändig information från de andra stegen importeras från den befintliga anslutningen.

En dataanslutning är datakällan som du importerar målgrupper från till Real-Time CDP Collaboration. För närvarande är Adobe Experience Platform den enda dataanslutning som stöds.

Alla inställningar, till exempel schemaläggning som du konfigurerar för dataanslutningen, tillämpas på alla målgrupper som importeras från den här dataanslutningen.
...
Om du vill lägga till dataanslutningen markerar du **[!UICONTROL Add a new data connection]** och väljer sedan **[!UICONTROL Next]**.
...
När du har valt datakälla måste du markera den sandlåda som innehåller de målgrupper som du vill importera. Markera sandlådan i listan över tillgängliga sandlådor och välj sedan **[!UICONTROL Next]**
...
Dessutom vill du välja dina kontextremsregler som ska gälla för data som importeras till Real-Time CDP Collaboration.
...
Därefter väljer du källfält som ska mappas till målfält i Real-Time CDP Collaboration.
...
**[!UICONTROL Source fields]** är identitetsnamnutrymmen och attribut från din befintliga implementering av Real-Time CDP. Detta är hur identiteterna finns i källan som du importerar data från. Source-fält mappas till målfälten som definierats i Real-Time CDP Collaboration.

**[!UICONTROL Target fields]** anger hur identiteterna refereras i Real-Time CDP Collaboration. För närvarande är hash-kodade e-postmeddelanden de enda matchningsnycklar som stöds.
...
Markera det tomma källfältet bredvid målfältet. Dialogrutan **[!UICONTROL Select source field]** visas. Välj mellan alternativen **[!UICONTROL Identity namespaces]** och **[!UICONTROL Profile attributes]** för att hitta det önskade källfältet och markera sedan källfältet i listan och använd sökalternativet för att hitta det önskade fältet.
...
När du har importerat målgrupper till Real-Time CDP Collaboration visas alla målgrupper som din organisation för närvarande har importerat till Real-Time CDP Collaboration på arbetsytan i **[!UICONTROL My audiences]**.
...
| **[!UICONTROL Status]** | Anger om målgruppen är aktiv och kan användas i projekt. Statusen **[!UICONTROL Pending]** anger att målgruppen nyligen har importerats och att målgruppsmedlemmarna ännu inte har fyllts i. De importerade målgrupperna fylls i med profiler efter den första uppdateringen, som vanligtvis sker inom 24 timmar efter att dataanslutningen har konfigurerats. |
...
| **[!UICONTROL Last updated]** | Anger det senaste datumet och den senaste tidpunkten då någon aspekt av målgruppen uppdaterades. |

![Arbetsytan Min målgrupp visar alla importerade målgrupper.](/help/assets/setup/add-manage-audiences/audiences-workspace.png)
...
| **[!UICONTROL Last updated]** | Anger det senaste datumet och den senaste tidpunkten då målgruppen uppdaterades. |
...
Avsnittet **[!UICONTROL Identities]** anger antalet profiler i målgruppen med någon av de identiteter du valde när du importerade målgruppen. Avsnittet innehåller också en identitetsuppdelning som gör att du kan se vilka identiteter som utgör det mesta av målgruppen.
...
En målgrupps tillgänglighet för användning i projekt med medarbetare skiljer sig åt beroende på inställningen för anslutningsåtkomst. I avsnittet **[!UICONTROL Connection access]** kan du välja om målgruppen ska vara privat, eller användbar och identifierbar i anslutningar.
...
>[!IMPORTANT]
>
>Oberoende av åtkomststatus (public, private eller custom) bidrar målgruppspopulationen till **[!UICONTROL All audiences]**-populationen i **[!UICONTROL Compare audiences]** -avsnittet i ett projekt.<br>
...
>abstract="<p>Anger vilka av målgruppens metadata som är synliga för andra organisationer innan de ansluter till din organisation. </p> <p> **Identitetsantal** kontrollerar om din partner kan visa identitetsantal för dina målgrupper när du visar överlappningsrapporter på identifieringsfliken. **Målgruppen överlappar %** kontrollerar om medarbetare kan identifiera överlappande procentandelar mellan sina målgrupper och dina."

>[!NOTE]
>
>Om din medarbetare har alla målgrupper inställda på privata, kommer **[!UICONTROL Relevant audiences]**-avsnittet i arbetsytan **[!UICONTROL Discover]** för ett projekt att vara tomt. Mer information finns i [Upptäck](/help/guide/collaborate/discover.md#relevant-audiences). guide.

Metadatasynlighet anger synligheten för en målgrupps metadata till andra organisationer innan de ansluter till din organisation, eller inom olika projektvyer. Om du vill uppdatera målgruppens metadatasynlighet väljer du alternativet **[!UICONTROL Edit]** i avsnittet **[!UICONTROL Metadata visibility]**.
...
**[!UICONTROL Show audience overlap %]**: När värdet är true kan medarbetare [upptäcka överlappande procentandelar](/help/guide/collaborate/discover.md#compare-audiences) mellan sina målgrupper och dina målgrupper.
...
När du har importerat målgrupper är det dags att identifiera utgivare så att de kan [ansluta](/help/guide/connect/establishing-connections.md) till och börja samarbeta i projekt.
