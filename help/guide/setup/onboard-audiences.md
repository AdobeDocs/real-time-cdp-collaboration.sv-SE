...
På fliken **[!UICONTROL My audiences]** i arbetsytan **[!UICONTROL Setup]** väljer du ikonen Lägg till (![ikonen Lägg till.](/help/assets/icons/plus.png)) och välj sedan **[!UICONTROL Audience]**. Om det här är din första målgrupp kan du även välja alternativet **[!UICONTROL Add]**.
...
* **CSV-fil**: Ladda upp en CSV-fil som innehåller målgruppsdata för snabb och enkel dataöverföring. Gå till guiden [Överför CSV-fil för att få information om målgruppskälla](./upload-csv-audience-sourcing.md) för att komma igång.
* **Amazon Web Services**: Anslut till din Amazon S3-lagring för att hämta målgruppsdata direkt från dina S3-bucket. I guiden [Konfigurera AWS S3 för målgruppskälla](./configure-aws-s3-audience-sourcing.md) finns stegvisa instruktioner.
* **Snowflake**: Använd Snowflake datalager för att hämta in målgruppsdata sömlöst. Se guiden [Konfigurera [!DNL Snowflake] för målgruppskälla](./configure-snowflake-audience-sourcing.md).
